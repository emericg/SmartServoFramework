...
protected:
    ...
    int m_servoSerie = SERVO_DRS;           //!< Servo serie using '::ServoDevices_e' enum. Used internally to setup some parameters like maxID, ackPolicy and protocolVersion.

    int m_protocolVersion = PROTOCOL_HKX;   //!< Version of the communication protocol in use.
    int m_maxId = 253;                      //!< Store in the maximum value for servo IDs.
    int m_ackPolicy = ACK_REPLY_READ;       //!< Set the status/ack packet return policy using '::AckPolicy_e' (0: No return; 1: Return for READ commands; 2: Return for all commands).
